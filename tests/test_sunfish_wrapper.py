...
def test_evaluation(engine):
    ...
    assert castle_analysis.king_safety > early_analysis.king_safety  # Castled king is safer
    ...
    engine.set_position(fen="8/8/8/8/3Q4/8/8/8 w - - 0 1")  # Queen in center
    center_analysis = engine.evaluate_position()
    engine.set_position(fen="Q7/8/8/8/8/8/8/8 w - - 0 1")  # Queen in corner
...
def test_board_utility_methods(engine):
    ...
    # A knight should attack 2 squares from the starting position
    g1_square = chess.parse_square("g1")
    attacked = engine.get_attacked_squares(g1_square)
    ...
    assert engine.is_attacked_by(chess.BLACK, e4_square)  # e4 is attacked by the d5 pawn
